...
_SIMPLIFIED_STRUCTURE_EXPLORER_DESCRIPTION_COMMENTS_ALT = """
A unified tool that traverses a code repository to retrieve dependency structure around specified directories, files, classes or functions with options to explore parents or children of the given directory, file, class or function, and control view depth and filters for choosing whether to get directories, files, classes or functions and relations like imports, invokes, contains and inherits. It also goes the extra step to extract and include relevant comments and docstrings from the code entities in the output and give class and function signatures.
"""


_SIMPLIFIED_TREE_EXAMPLE_COMMENTS_ALT = """
Example Usage:
1. Exploring Downstream Dependencies:
    ```
...
_DETAILED_STRUCTURE_EXPLORER_DESCRIPTION_COMMENTS_ALT = """
Unified code repository exploring tool that traverses a code repository to retrieve dependency structure around specified directory, file, class or functions.
The direction of search can be controlled to traverse upstream (exploring dependencies that directory, file, class or functions rely on) or downstream (exploring how directory, file, class or functions impact others), with optional limits on traversal depth and filters for directory, file, class or functions and dependency types. It also goes the extra step to extract and include relevant comments and docstrings from the code entities in the output and give class and function signatures.
...
_DETAILED_TREE_EXAMPLE_COMMENTS_ALT = """
Example Usage:
1. Exploring downward dependencies:
    ```
    get_code_structure_overview_with_code_comments(
...
_STRUCTURE_EXPLORER_PARAMETERS_COMMENTS_ALT = {
    'type': 'object',
    'properties': {
        'start': {
            'description': (
...
def create_explore_code_structure_tool_with_comments(
    use_simplified_description: bool = False,
) -> ChatCompletionToolParam:
    description = (
        _SIMPLIFIED_STRUCTURE_EXPLORER_DESCRIPTION_COMMENTS_ALT
        if use_simplified_description
        else _DETAILED_STRUCTURE_EXPLORER_DESCRIPTION_COMMENTS_ALT
    )
    example = (
        _SIMPLIFIED_TREE_EXAMPLE_COMMENTS_ALT
        if use_simplified_description
        else _DETAILED_TREE_EXAMPLE_COMMENTS_ALT
    )
    return ChatCompletionToolParam(
        type='function',
        function=ChatCompletionToolParamFunctionChunk(
            name='get_code_structure_overview_with_code_comments',
            description=description + example,
            parameters=_STRUCTURE_EXPLORER_PARAMETERS_COMMENTS_ALT,
        ),
    )
