...
class DockerRuntime(ActionExecutionClient):
    ...
    def maybe_build_runtime_container_image(self):
        ...
        prefix, suffix = self.get_info_from_base_image()
        try:
            runtime_docker_image_folder = self.find_folder_by_repo_name(
                prefix, suffix, "/workspaces/Openhands/runtime_dockers/swebench_dev/"
            )
        except:
            runtime_docker_image_folder = self.find_folder_by_repo_name(
                prefix, suffix, "/workspaces/Openhands/runtime_dockers/swebench_verified/"
            )
    ...
    def find_folder_by_repo_name(self, prefix, suffix, search_dir):
        matches = []
        for p in Path(search_dir).iterdir():
            if p.is_dir():
                if prefix in p.name and suffix in p.name:
                    matches.append(p)
        if matches:
    ...
    def get_info_from_base_image(self):
        if self.base_container_image.startswith("docker.io"):
            prefix = self.base_container_image.split(":")[0].split("/")[-1].split(".")[-1].split("_1776_")[0]
            suffix = self.base_container_image.split(":")[0].split("/")[-1].split(".")[-1].split("_1776_")[1]
            return prefix, suffix
        elif self.base_container_image.startswith("openhands_local"):
            prefix = self.base_container_image.split(":")[0].split("openhands_local_")[1].split("-")[0]
            suffix = self.base_container_image.split(":")[0].split("openhands_local_")[1].split("-")[1]
            return prefix, suffix
        else:
            raise ValueError(
